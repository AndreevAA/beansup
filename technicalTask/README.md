...
Кнопка "Добавить" открывает список по добавлению выбора Карта/Депозит/Наличные, где пользователь вводит необходимые данные о карте и добавляет ее. Описано в пункте `Добавление продукта`.
...
После выбора нужной карты, пользователю открывается меню `Форма заполнения данных о карте`, после успешного заполнения каторой, карта добавляется в портфель пользователя и отображется в списке продуктов, а окно добавления закрывается.


### Добавление депозита
При выборе "Депозит", открывается `Форма заполнения данных о депозите`, после успешного заполнения которой депозит добавляется в список продуктов пользователя, а окно добавления закрывается.

### Добавление наличных
При выборе "Наличные", открывается `Форма заполнения данных о наличных`, после успешного заполнения которой наличные добавляются в список продуктов пользователя, а окно добавления закрывается.
